...
export default async function Home() {
  ...
  const itemMap = new Map<string, FeedVideo>(videoRows.map(v => [v.id, { id: v.id, title: v.title, slug: v.slug, category: v.category?.[0]?.name ?? 'Culture', views: views(countMap.get(v.id) ?? 0), age: ago(v.published_at), duration: duration(v.duration_seconds), thumbnailUrl: v.thumbnail_path ? supabase.storage.from('thumbnails').getPublicUrl(v.thumbnail_path).data.publicUrl : null }]))
