...
export async function importYoutubeDraft(formData: FormData) {
  ...
  if (process.env.YOUTUBE_API_KEY) {
    ...
    if (response.ok) { const payload = await response.json() as { items?: Array<{ snippet?: { title?: string; description?: string; channelTitle?: string }; contentDetails?: { duration?: string } }> }; const metadata = payload.items?.[0]; if (!metadata) redirect('/admin/youtube?error=YouTube%20could%20not%20find%20that%20video'); title ||= metadata.snippet?.title?.trim() ?? ''; description ||= metadata.snippet?.description?.trim() ?? ''; duration = seconds(metadata.contentDetails?.duration) }
  }
  ...
  const { data, error } = await supabase.from('videos').insert({ title, description: description || null, slug, source: 'youtube', provider_video_id: id, duration_seconds: duration, status: 'draft', created_by: user.id, updated_by: user.id }).select('id').single()
